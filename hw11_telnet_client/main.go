...
func main() {
	...
	//важно для последней строки!!! создаем именно БУФФЕРИЗОВАННЫЙ канал (для не блокирующей записи)
	//
	...
	//тут происходит блокирующее чтение:
	<-done //читаем из канала, а точнее "блокируем" главный поток, пока хотя бы одна из горутин не записала туда
}
